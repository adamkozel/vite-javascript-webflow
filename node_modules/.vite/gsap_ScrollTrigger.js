...
var _addListener = function _addListener2(element, type, func, nonPassive) {
...
var _addListener3 = function _addListener4(element, type, func, nonPassive) {
  return element.addEventListener(type, func, {
    passive: !nonPassive
  });
};
...
var _onResize = function _onResize2() {
  _scrollers.cache++;
  !_refreshing && !_ignoreResize && !_doc2.fullscreenElement && (!_ignoreMobileResize || _baseScreenWidth !== _win2.innerWidth || Math.abs(_win2.innerHeight - _baseScreenHeight) > _win2.innerHeight * 0.25) && _resizeDelay.restart(true);
};
var _listeners = {};
var _emptyArray = [];
var _media = [];
...
var ScrollTrigger2 = function() {
    ...
    if (snap) {
      ...
      snapDelayedCall = gsap2.delayedCall(snap.delay || scrubSmooth / 2 || 0.1, function() {
        if (Math.abs(self.getVelocity()) < 10 && !_pointerIsDown && lastSnap !== scrollFunc()) {
          var totalProgress = animation && !isToggle ? animation.totalProgress() : self.progress, velocity = (totalProgress - snap2) / (_getTime2() - _time2) * 1e3 || 0, change1 = gsap2.utils.clamp(-self.progress, 1 - self.progress, _abs(velocity / 2) * velocity / 0.185), naturalEnd = self.progress + (snap.inertia === false ? 0 : change1), endValue = _clamp2(0, 1, snapFunc(naturalEnd, self)), scroll = scrollFunc(), endScroll = Math.round(start + endValue * change), _snap = snap, onStart = _snap.onStart, _onInterrupt = _snap.onInterrupt, _onComplete = _snap.onComplete, tween = tweenTo.tween;
  ...
  ScrollTrigger3.enable = function enable() {
    ...
    if (gsap2) {
      ...
      if (_body2) {
        _enabled = 1;
        ScrollTrigger3.isTouch = _win2.matchMedia && _win2.matchMedia("(hover: none), (pointer: coarse)").matches ? 1 : "ontouchstart" in _win2 || navigator.maxTouchPoints > 0 || navigator.msMaxTouchPoints > 0 ? 2 : 0;
...
var _allowNativePanning = function _allowNativePanning2(direction) {
  if (direction === true) {
    _body2.style.removeProperty("touch-action");
    _docEl2.style.removeProperty("touch-action");
  } else {
    _body2.style.touchAction = _docEl2.style.touchAction = direction ? "pan-" + direction : "none";
  }
};
var _getScrollNormalizer = function _getScrollNormalizer2(vars) {
  ...
  var _vars2 = vars, normalizeScrollX = _vars2.normalizeScrollX, momentum = _vars2.momentum, self, maxY, onClickTime = 0, scrollFuncY = _getScrollFunc(_docEl2, _vertical), scrollFuncX = _getScrollFunc(_docEl2, _horizontal), scale = 1, resolveMomentumDuration = _isFunction(momentum) ? momentum(self) : function() {
    return momentum || 2.8;
  }, skipTouchMove, lastRefreshID, onClick = function onClick2() {
    ...
    maxY = _maxScroll(_docEl2, _vertical);
    scrollClampY = _clamp2(0, maxY);
    normalizeScrollX && (scrollClampX = _clamp2(0, _maxScroll(_docEl2, _horizontal)));
    ...
    skipTouchMove = true;
  }, tween, startScrollX, startScrollY, ownerDoc, onStopDelayedCall, onResize = function onResize2() {
    updateClamps();
    tween.isActive() && tween.vars.scrollY > maxY && tween.resetTo("scrollY", _maxScroll(_docEl2, _vertical));
  };
  ...
  vars.onPress = function() {
    var prevScale = scale;
    scale = _win2.visualViewport && _win2.visualViewport.scale || 1;
    tween.pause();
  ...
  vars.onRelease = vars.onGestureStart = function(self2, wasDragging) {
    var e = self2.event, eventData = e.changedTouches ? e.changedTouches[0] : e;
    if (!wasDragging || Math.abs(self2.x - self2.startX) <= 3 && Math.abs(self2.y - self2.startY) <= 3) {
      ...
      if (normalizeScrollX) {
        ...
        endScroll = currentScroll + dur * 0.05 * -self2.velocityX / 0.227 / scale;
        dur *= _clampScrollAndGetDurationMultiplier(scrollFuncX, currentScroll, endScroll, _maxScroll(_docEl2, _horizontal));
        tween.vars.scrollX = scrollClampX(endScroll);
      }
      currentScroll = scrollFuncY();
      endScroll = currentScroll + dur * 0.05 * -self2.velocityY / 0.227 / scale;
      dur *= _clampScrollAndGetDurationMultiplier(scrollFuncY, currentScroll, endScroll, _maxScroll(_docEl2, _vertical));
  ...
  vars.onChange = function(self2, dx, dy, xArray, yArray) {
    ...
    dx && normalizeScrollX && scrollFuncX(scrollClampX(xArray[2] === dx ? startScrollX + (self2.startX - self2.x) / scale : scrollFuncX() + dx - xArray[1]));
    dy && scrollFuncY(scrollClampY(yArray[2] === dy ? startScrollY + (self2.startY - self2.y) / scale : scrollFuncY() + dy - yArray[1]));
    _updateAll();
  };
  vars.onEnable = function(self2) {
    _allowNativePanning(normalizeScrollX ? false : "x");
    _addListener3(_win2, "resize", onResize);
    self2.target.addEventListener("click", onClick, {
      capture: true
    });
  };
  vars.onDisable = function(self2) {
    _allowNativePanning(true);
    _removeListener3(_win2, "resize", onResize);
    _removeListener3(self2.target, "click", onClick);
  };
  self = new Observer(vars);
  ownerDoc = self.target.ownerDocument || _doc2;
...
ScrollTrigger2.normalizeScroll = function(vars) {
  ...
  _normalizer2 && (vars === false || isInstance && vars !== _normalizer2) && _normalizer2.kill();
  vars && !isInstance && (vars = _getScrollNormalizer(vars));
  return _normalizer2 = vars && vars.enable();
};
ScrollTrigger2.core = {
  _getVelocityProp,
  _scrollers,
