...
// Select all elements with the class name "left"
const elements = document.querySelectorAll('.left')

// Loop through each element and attach the event listeners
elements.forEach((element) => {
  // Create a mouseenter event listener
  element.addEventListener('mouseenter', () => {
    // Calculate the new size based on current size + 10%
    const currentSize = element.getBoundingClientRect().width
    const newSize = currentSize * 1.01

    // Convert the new size to rem units
    const newSizeRem = `${newSize / 20}rem`

    // Animation on mouseenter
    gsap.to(element, {
      ease: 'power4.out',
      duration: 1,
      gridColumnGap: newSizeRem,
    })
  })
  ...
  element.addEventListener('mouseleave', () => {
    ...
    gsap.to(element, {
      duration: 1,
      gridColumnGap: '8.5rem',
      ease: 'expo.out',
    })
  })
})

// Select all elements with the class name "text services"
const elementsServices = document.querySelectorAll('.text.services')
...
elementsServices.forEach((element) => {
  ...
  element.addEventListener('mouseleave', () => {
    ...
    gsap.to(element, { x: 0, ease: 'expo.out' })
  })
})
